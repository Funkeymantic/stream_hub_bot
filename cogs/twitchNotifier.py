...
class TwitchNotifier(commands.Cog):
    ...
    async def cog_load(self):
        ...
        print("[TwitchNotifier] Cog loaded and stream check started.")
    ...
    async def refresh_twitch_users(self):
        ...
        if not self.bot.guilds:
            print("[TwitchNotifier] No guilds connected yet.")
            return
        guild = self.bot.guilds[0]  # Assumes single guild
        ...
        for streamer_name, member in self.streamers.items():
            ...
            if is_live:
                if streamer_name not in self.live_messages and channel:
                    msg = await channel.send(f"{streamer_name} is now live! https://twitch.tv/{streamer_name}")
                    self.live_messages[streamer_name] = msg.id
            else:
                if streamer_name in self.live_messages and channel:
                    try:
                        msg = await channel.fetch_message(self.live_messages.pop(streamer_name))
                        await msg.delete()
